...
import { addoverview, addtitle, toggleMute } from '../utils/movieSlice';
...
const VideoTitle = ({title, overview}) => {
    ...
    <div className='w-screen aspect-video  pt-[14%] px-2 md:px-24 absolute text-white bg-gradient-to-r from-black'>
        ...
        <h1 className='text-xl md:text-3xl font-bold'>{movie.title}</h1>
            ...
            <button className='rounded-full w-10 h-10 p-1 bg-slate-100 text-black hover:opacity-80' onClick={handleMute}>
            <FontAwesomeIcon className='mr-1' icon={faVolumeXmark} />
            </button>
        )}

        </div>
        <p className='hidden md:inline-block py-3 text-lg w-2/4'>{movie.overview}</p>
